...
## 2026-07-24 — 지역 입력 정합화: 해외 출생 시간대 환산

지역 입력 전 경로 실측에서 전세계 도시 검색 UI와 한국 벽시계 전제 엔진의 결합 결함을 발견했다 —
뉴욕 현지 14:30 출생이 (현지경도 − 한국 자오선)×4분 = −836분 오보정으로 시주가 자시로 오판됐다.
도시의 IANA 시간대로 현지→한국 벽시계 환산 후 기존 파이프라인을 태우는 최소 수정으로 마감 —
환산 후 기존 보정이 수학적으로 현지 시태양시와 동일해 유파 선택을 새로 강요하지 않는다.

- based_on: standalone (사용자 요청: 지역 입력 점검)
- changes: 13 files, +282 −13 / `ac452ed` + 후속: 역사 시대 정합(KSTHistory.utc_to_wall_clock,
  IANA→SSOT 교체, 왕복 스윕 37만 샘플 중 fold 8건만, 테스트 +7 → pytest 351)
- why: 서비스가 1급으로 노출하는 입력 경로(폴백 목록에도 뉴욕·런던)가 무의미한 명식을 내는
  정합성 결함. 부수로 동명 도시 이름 재조회 오해석도 선택 도시의 좌표·시간대 명시 전송으로 차단
- verification: 뉴욕 진태양시 13:37 수기 검산 일치·시주 계미 정정 · 서울 완전 불변 · 홍콩 자정
  경계 왕복 · pytest 344(+11) · ruff · npm run gate 통과
- category: devlog
- companion: [.claude/devlog/2026-07-24_11-36-15_foreign-birth-timezone.md](./2026-07-24_11-36-15_foreign-birth-timezone.md)
