...
- ~~**절기 경계 판정의 시스템 오프셋**~~ (해결됨): 절기 시각이 실제 천문 시각과 어긋나던 원인은 두 가지였다 — (1) ephem이 naive datetime을 UTC로 해석하는데 KST 변환 없이 그대로 쓰던 것(−9h), (2) `ephem.Ecliptic`이 기본 J2000 분점 기준 황경을 반환해 세차(≈50.3″/년)만큼 어긋나던 것(2024년 기준 약 +8h, 1990년 기준 약 −3h). 현재는 `manseol/core/astronomical.py`가 KST↔UTC 변환과 of-date 분점 황경을 사용해 KASI 공표 절입 시각과 ±1분 이내로 정합하며(1954~1961 UTC+8:30 시기는 당시 표준시로 반환), `tests/unit/manseol/test_solar_terms_absolute.py`가 이를 회귀 고정한다.
- **절기 경계 비교의 진태양시 규약**: 기본 설정은 출생 시각을 진태양시(경도 보정 −32분 + 균시차)로 바꾼 뒤 KST 기준 절입 시각과 비교한다. 절입 직전·직후 ±40분 안쪽에서는 이 규약(진태양시파 vs 표준시파)에 따라 월주가 갈릴 수 있으며, 이는 명리 유파 간 통설이 갈리는 영역이라 옵션(`--no-time-correction`)으로 남긴다.
- **해외 출생은 현지 시각으로 입력**: 출생 도시가 한국 밖이면 도시의 IANA 시간대(또는 API/CLI의 `timezone` 명시 입력)로 현지 시각을 한국 벽시계로 환산한 뒤 계산한다. 환산 후 진태양시 보정 (현지경도 − 135°)×4분은 수학적으로 현지 시태양시와 일치하므로 시주·일주는 현지 태양시 기준, 절기 판정은 절대 동시성 기준으로 성립한다(`tests/unit/manseol/test_timezone_conversion.py`). 예외: `--longitude`만 단독 지정하면 입력 시각을 한국 시각으로 간주하는 기존 규약을 유지한다.
