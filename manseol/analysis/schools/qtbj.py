...
from ..yongsin.base import WuXing, get_sheng_me_element, get_ke_me_element
...
class QTBJInterpreter(BaseSchoolInterpreter):
    ...
    def interpret_health(self, saju_data: Dict[str, Any], yong_sin: WuXing) -> str:
        ...
        health_text = f"궁통보감에서는 {season_name}에 태어난 {day_element.value} 일간의 건강을 중시합니다. "
    ...
    def interpret_wealth(self, saju_data: Dict[str, Any], yong_sin: WuXing) -> str:
        ...
        if yong_sin_ratio > 0.15:
            wealth_text += f"{yong_sin.value} 오행이 있어 계절의 부조화를 보완하고 재물운이 순탄합니다. "
        else:
