...
class SchoolComparator:
    ...
    def compare_schools(
        ...
        schools: Optional[List[SchoolCode]] = None
    ) -> SchoolComparisonResult:
        """
        여러 유파로 해석 및 비교
    ...
    def _find_consensus(
        ...
        for category in categories:
            common_keywords = SchoolComparator._find_common_keywords(
            ...
            if common_keywords:
                agreeing_schools = [
                    ...
                    for interp in interpretations
                    if any(
                        keyword in getattr(interp, category, "")
    ...
    def _find_common_keywords(
        ...
        # 2개 이상 유파에서 등장하는 키워드
        common = [
            word for word, count in keywords.items()
            if count >= 2
        ]
    ...
    def _generate_recommendation(
        ...
        if high_confidence:
            ...
            lines.append("")
            lines.append(
                f"가장 신뢰도 높은 해석: {best.school_name} "
                f"({best.confidence * 100:.0f}%)"
            )
