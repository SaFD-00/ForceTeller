...
from config.constants import BRANCHES, STEMS, Polarity
...
class FortuneCycleCalculator:
    ...
    def calculate_daewun_start_age(self) -> tuple[int, int, int]:
        ...
        birth_date = (
            self.birth_datetime.date()
            if isinstance(self.birth_datetime, datetime)
        ...
        # 일수 차이 계산
        birth_dt = (
            datetime.combine(birth_date, self.birth_datetime.time())
            if isinstance(self.birth_datetime, datetime)
            else datetime.combine(birth_date, datetime.min.time())
        )
    ...
    def get_current_daewun(self, current_age: int = None) -> dict | None:
        """
        현재 대운 반환
