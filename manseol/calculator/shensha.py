...
# 홍염살 - 일간 기준 특정 지지 (고전 시결 「甲午乙午丙寅辛見酉, 戊己怕辰壬怕子, 庚見戌兮癸見申, 丁人見未多風情」 기준)
# 갑->오, 을->오, 병->인, 정->미, 무->진, 기->진, 경->술, 신->유, 임->자, 계->신
# 주: 을(乙)은 판본에 따라 午(다수설) 또는 申(변형)을 쓴다. 여기서는 가이드(Interpretation.md)에 맞춰 午로 통일.
HONGYEOM_TABLE = {
    0: [6],  # 갑 -> 오
    1: [6],  # 을 -> 오
    2: [2],  # 병 -> 인
...
class ShenshaCalculator:
    ...
    def __init__(
        self, year_pillar: tuple, month_pillar: tuple, day_pillar: tuple, hour_pillar: tuple = None
    ):
        """
        Args:
        ...
        # 삼합 그룹 결정 (년지 기준)
        self.samhap_group = (
            SAMHAP_GROUPS.get(self.year_branch) if self.year_branch is not None else None
        )
    ...
    def _check_samhap_based_shensha(self) -> list[dict]:
        ...
        shensha_info = {
            "겁살": {
                ...
                "type": "흉신",
                "hanja": "劫殺",
                "description": SHENSHA.get("겁살", {}).get(
                    "description", "경쟁과 시비, 재물 손실 우려, 절지(絶地)에 해당"
                ),
            },
            "재살": {
                "name": "재살 (災殺)",
                "type": "흉신",
                "hanja": "災殺",
                "description": SHENSHA.get("재살", {}).get(
                    "description", "재앙과 질병의 별, 태지(胎地)에 해당"
                ),
            },
            "천살": {
                "name": "천살 (天殺)",
                "type": "흉신",
                "hanja": "天殺",
                "description": SHENSHA.get("천살", {}).get(
                    "description", "하늘의 재앙, 양지(養地)에 해당, 예기치 못한 사고"
                ),
            },
            "지살": {
                "name": "지살 (地殺)",
                "type": "흉신",
                "hanja": "地殺",
                "description": SHENSHA.get("지살", {}).get(
                    "description", "땅의 재앙, 장생지(長生地)에 해당, 부동산/토지 관련 흉"
                ),
            },
            "연살": {
                "name": "연살 (年殺)",
                "type": "흉신",
                "hanja": "年殺",
                "description": SHENSHA.get("연살", {}).get(
                    "description", "해당 년의 액운, 목욕지에 해당"
                ),
            },
            "월살": {
                "name": "월살 (月殺)",
                "type": "흉신",
                "hanja": "月殺",
                "description": SHENSHA.get("월살", {}).get(
                    "description", "해당 월의 액운, 관대지에 해당"
                ),
            },
            "망신살": {
                "name": "망신살 (亡身殺)",
                "type": "흉신",
                "hanja": "亡身殺",
                "description": SHENSHA.get("망신", {}).get(
                    "description", "체면과 명예 관련 구설수, 자신을 드러냄, 임관지에 해당"
                ),
            },
            "장성살": {
                "name": "장성살 (將星殺)",
                "type": "길신",
                "hanja": "將星殺",
                "description": SHENSHA.get("장성", {}).get(
                    "description", "왕성한 기운, 리더십과 권력의 별, 제왕지에 해당"
                ),
            },
            "반안살": {
                "name": "반안살 (攀鞍殺)",
                "type": "길신",
                "hanja": "攀鞍殺",
                "description": SHENSHA.get("반안살", {}).get(
                    "description", "안장에 오름, 승진과 발전의 별, 쇠지(衰地)에 해당"
                ),
            },
            "역마살": {
                "name": "역마살 (驛馬殺)",
                "type": "중성",
                "hanja": "驛馬殺",
                "description": SHENSHA.get("역마", {}).get(
                    "description", "이동과 변동이 많은 별, 잦은 이동수, 병지(病地)에 해당"
                ),
            },
            "육해살": {
                "name": "육해살 (六害殺)",
                "type": "흉신",
                "hanja": "六害殺",
                "description": SHENSHA.get("육해살", {}).get(
                    "description", "육친과의 갈등, 해로운 관계, 사지(死地)에 해당"
                ),
            },
            "화개살": {
                "name": "화개살 (華蓋殺)",
                "type": "길신",
                "hanja": "華蓋殺",
                "description": SHENSHA.get("화개", {}).get(
                    "description", "학문, 예술, 종교적 심성의 별, 묘지(墓地)에 해당"
                ),
            },
            ...
            "도화": {
                ...
                "type": "중성",
                "hanja": "桃花殺",
                "description": SHENSHA.get("도화", {}).get(
                    "description", "이성에게 매력을 끼치는 별, 색정과 연애 운"
                ),
            },
        }
                ...
                result.append(
                    {
                        ...
                        "type": "길신",
                        "position": pillar,
                        "description": SHENSHA.get("천을귀인", {}).get(
                            "description", "귀인의 도움을 받음, 위기에서 구원받음"
                        ),
                    }
                )
                ...
                result.append(
                    {
                        ...
                        "type": "길신",
                        "position": pillar,
                        "description": SHENSHA.get("문창귀인", {}).get(
                            "description", "학문, 시험 운이 좋음, 글재주"
                        ),
                    }
                )
                ...
                result.append(
                    {
                        ...
                        "type": "중성",
                        "position": pillar,
                        "description": SHENSHA.get("홍염살", {}).get(
                            "description", "능동적 매력, 타겟에게 의도적 매력 발산"
                        ),
                    }
                )
                    ...
                    result.append(
                        {
                            ...
                            "type": "흉신",
                            "position": pillar,
                            "description": SHENSHA.get("양인살", {}).get(
                                "description",
                                "날카로운 칼날의 별, 강한 기운이나 재앙 우려, 제왕지에 해당",
                            ),
                        }
                    )
                    ...
                    result.append(
                        {
                            ...
                            "type": "길신",
                            "position": pillar,
                            "description": SHENSHA.get("학당귀인", {}).get(
                                "description", "학문에 뛰어난 재능, 총명함, 장생지에 해당"
                            ),
                        }
                    )
                    ...
                    result.append(
                        {
                            ...
                            "type": "길신",
                            "position": pillar,
                            "description": SHENSHA.get("금여록", {}).get(
                                "description", "금수레의 별, 명예와 부귀를 누림, 귀한 배우자"
                            ),
                        }
                    )
    ...
    def _check_day_branch_shensha(self) -> list[dict]:
        ...
        # 신-해, 유-술, 술-유, 해-신
        wonjin_pairs = {0: 7, 1: 6, 2: 5, 3: 4, 4: 3, 5: 2, 6: 1, 7: 0, 8: 11, 9: 10, 10: 9, 11: 8}
                ...
                result.append(
                    {
                        ...
                        "type": "흉신",
                        "position": pillar,
                        "description": SHENSHA.get("원진", {}).get(
                            "description", "인연의 괴로움, 서로 불편한 관계"
                        ),
                    }
                )
        ...
        gwimun_pairs = {0: 9, 1: 10, 2: 7, 3: 6, 4: 5, 5: 4, 6: 3, 7: 2, 8: 1, 9: 0, 10: 11, 11: 8}
                ...
                result.append(
                    {
                        ...
                        "type": "흉신",
                        "position": pillar,
                        "description": SHENSHA.get("귀문관살", {}).get(
                            "description", "정신적 고민, 신비 체험, 예민한 감각"
                        ),
                    }
                )
                    ...
                    result.append(
                        {
                            ...
                            "type": "흉신",
                            "position": pillar,
                            "description": SHENSHA.get("백호살", {}).get(
                                "description", "흰 호랑이의 별, 혈광지사 우려, 외과적 수술 암시"
                            ),
                        }
                    )
                ...
                result.append(
                    {
                        ...
                        "type": "길신",
                        "position": "사주",
                        "description": samgi_descriptions.get(
                            samgi_name, "세 가지 기이한 별, 특별한 재능과 복"
                        ),
                    }
                )
                    ...
                    result.append(
                        {
                            ...
                            "type": "길신",
                            "position": pillar,
                            "description": SHENSHA.get("천덕귀인", {}).get(
                                "description", "하늘의 덕을 받는 귀인, 재난을 피하고 복을 받음"
                            ),
                        }
                    )
                    ...
                    result.append(
                        {
                            ...
                            "type": "길신",
                            "position": pillar,
                            "description": SHENSHA.get("월덕귀인", {}).get(
                                "description", "달의 덕을 받는 귀인, 평화롭고 화합함"
                            ),
                        }
                    )
