    ...
    try:
        ...
        if birth_time:
            try:
                ...
                console.print("[red]오류: 시간 형식이 잘못되었습니다. HH:MM 형식으로 입력하세요.[/red]")
                return
...
def _display_table(result):
    ...
    # 신강/신약
    strength_color = "green" if result.analysis.strength.level == "신강" else \
                     "red" if result.analysis.strength.level == "신약" else "yellow"
    console.print(f"\n신강/신약: [{strength_color}]{result.analysis.strength.level}[/{strength_color}] "
                 f"(점수: {result.analysis.strength.score})")

    # 용신
    console.print(f"용신: [bold]{result.analysis.useful_god.primary}[/bold] ({result.analysis.useful_god.type})")
