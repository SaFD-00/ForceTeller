...
class BranchData(BaseModel):
    ...
    hidden_stems: list[dict[str, Any]] = Field(default_factory=list, description="지장간 정보")
...
class PillarData(BaseModel):
    ...
    ten_god: str | None = Field(None, description="십성 (일주 제외)")
    twelve_phase: str | None = Field(None, description="12운성")
...
class FourPillars(BaseModel):
    ...
    hour: PillarData | None = Field(None, description="시주 (시간 미상시 None)")
...
class DayMasterAnalysis(BaseModel):
    ...
    characteristics: list[str] = Field(default_factory=list, description="특성 키워드")
...
class FiveElementsAnalysis(BaseModel):
    ...
    distribution: dict[str, float] = Field(default_factory=dict, description="오행 비율 (%)")
...
class UsefulGodAnalysis(BaseModel):
    ...
    secondary: str | None = Field(None, description="희신 오행")
    avoid: str | None = Field(None, description="기신 오행")
...
class FortuneCycleData(BaseModel):
    ...
    cycles: list[FortuneCycle] = Field(default_factory=list, description="대운 목록 (10개)")
    current_cycle_index: int | None = Field(None, description="현재 대운 인덱스")
...
class SajuAnalysis(BaseModel):
    ...
    shensha: list[ShenshaData] = Field(default_factory=list, description="신살 목록")
...
class InputSummary(BaseModel):
    ...
    jajasi: bool
    # 음력 정보
    lunar_year: int | None = Field(None, description="음력 연도")
    lunar_month: int | None = Field(None, description="음력 월")
    lunar_day: int | None = Field(None, description="음력 일")
    is_leap_month: bool | None = Field(None, description="윤달 여부")
    # 일주 정보
    day_ganji_korean: str | None = Field(None, description="일주 한글 (예: 경진)")
    day_ganji_chinese: str | None = Field(None, description="일주 한자 (예: 庚辰)")
    day_metaphor: str | None = Field(None, description="일주 물상 (예: 하얀 용)")
    day_animal: str | None = Field(None, description="일지 동물 (예: 용)")


class SajuResult(BaseModel):
    """사주 계산 최종 결과"""

    meta: MetaInfo = Field(default_factory=MetaInfo, description="메타 정보")

    input: InputSummary = Field(..., description="입력 요약")

    adjusted_time: TimeCorrection | None = Field(None, description="시간 보정 정보")

    pillars: FourPillars = Field(..., description="사주 4주")

    analysis: SajuAnalysis = Field(..., description="사주 분석")

    fortune_cycles: FortuneCycleData | None = Field(None, description="대운 데이터")

    interactions: dict[str, list[dict[str, Any]]] | None = Field(
        None, description="천간/지지 상호작용 (합·충·형·파·해·공망)"
    )
