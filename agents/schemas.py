...
class InterpretationResult(BaseModel):
    ...
    confidence: float = Field(default=1.0, ge=0, le=1, description="해석 신뢰도 (0.0 ~ 1.0)")
...
class SynthesisResult(BaseModel):
    ...
    key_insights: list[str] = Field(default_factory=list, description="핵심 인사이트 요약 (3-5개)")
    suggested_questions: list[str] = Field(default_factory=list, description="추가 탐색 질문")
