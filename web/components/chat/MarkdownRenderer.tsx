...
export function MarkdownRenderer({ content }: MarkdownRendererProps) {
  const components: Components = {
    ...
    code: ({ className, children, ...props }) => {
      ...
      return (
        <code className="block bg-muted p-3 rounded-lg border-[1.5px] border-border my-2 overflow-x-auto font-mono text-sm text-foreground" {...props}>
          {children}
        </code>
      );
    },
    ...
    // 구분선
    hr: () => (
      <hr className="my-4 border-t-[1.5px] border-border" />
    ),
    ...
    table: ({ children }) => (
      <div className="overflow-x-auto my-3">
        <table className="min-w-full border-[1.5px] border-border rounded-lg overflow-hidden">
          {children}
        </table>
      </div>
    ),
