...
| `curious` | 설명봇(`GlossaryTooltip`) 헤더 |
| `idle` | 사이드바 로고, 일반 브랜딩 |
| `sleeping` | 비활성/세션 종료 등(드묾) |

## 색
스케치 잉크 윤곽 `#263D5B`(= `border` 토큰), 고깔모자 스카이 크레용 `#49B6E5`(= `primary` 토큰, 채움 전용),
모자 챙·생각점 `#2E8CB5`(SKY 파생 다크 — 백색 대비 3.5:1로 non-text AA 충족), 별 옐로우 `#ffd23f`, 달 얼굴 흰색.
색은 컴포넌트에 hex 상수(`INK`/`SKY`/`SKY_DK`)로 박혀 앱과 design-sync 헤드리스 렌더에서 동일하게 보인다(토큰 변수 의존 X).
`web/app/icon.svg`(파비콘)도 같은 팔레트를 쓴다 — 마스코트 색을 바꾸면 파비콘도 함께 갱신할 것.
