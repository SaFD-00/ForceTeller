...
- result/chat 의 표·차트·리스트형 컴포넌트는 그리드 셀보다 넓어 `[GRID_OVERFLOW]` → `cfg.overrides.<Name>:
  {"cardMode":"column"}` 적용. 2026 tetris 리디자인 당시 17개(Mascot, MessageBubble, AgentSelector,
  SuggestedQuestions, ElementDistribution, FiveElementsChart, FortuneCycleTimeline, FortuneScoreDashboard,
  FourPillarsDisplay, InteractionsTabs, LuckyGuideCard, PentagonChart, SchoolComparison, ShenshaDetailCard,
  StrengthMeter, TenGodsDistribution, YongshinCard)에 적용했으나, 이후 result 컴포넌트가 개편되며
  FiveElementsChart·FortuneCycleTimeline·FourPillarsDisplay·StrengthMeter·TenGodsDistribution 은 컴포넌트로는
  더 이상 존재하지 않는다(FourPillarsDisplay·TenGodsDistribution 은 `types/saju.ts`의 타입으로만 남음).
  **현재** `config.json` 의 override 는 그 5개를 뺀 12개이며, column 은 wide 로 re-flag 불가하니 재검증 불필요.
- (2026 tetris 리디자인 당시) recharts(FiveElementsChart·PentagonChart 일부)는 프리뷰 wrapper를 고정폭/높이로
  감싸야 0-height 를 방지했다 — **현재** result 컴포넌트는 recharts 를 쓰지 않는다(SVG/커스텀 렌더로 전환).
