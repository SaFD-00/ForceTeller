...
export function Moods() {
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 20, padding: 24, background: "#dfe7ff" }}>
      {moods.map((m) => (
        <div key={m} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
          <Mascot mood={m} size="lg" />
          <code style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "#1c202b" }}>{m}</code>
        </div>
      ))}
    </div>
  );
...
export function Sizes() {
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 20, padding: 24, background: "#dfe7ff" }}>
...
export function WithBubble() {
  return (
    <div style={{ padding: 24, background: "#dfe7ff", width: 420 }}>
