// forked from design-sync lib/bundle.mjs — ForceTeller web is a Next.js 14 app
// bundled via synth-entry (no dist). Two browser-runtime fixes baked into the IIFE:
//  (1) process shim: app code (lib/api/client.ts, lib/api/chat.ts) reads
//      process.env.NEXT_PUBLIC_API_URL and bundled remark/vfile polyfills
//      reference bare `process`, undefined in the browser → `ReferenceError:
//      process is not defined` on every preview (and every claude.ai/design render).
//  (2) next/navigation no-op stub: Sidebar calls usePathname(), BirthInfoForm calls
//      useRouter(); outside a Next app both throw "invariant expected app router to
//      be mounted". The stub returns inert values so every component renders.
// Both sit inside the IIFE (process shim as a banner below stampHeader's
// `/* @ds-bundle */` line, above the window.<GLOBAL>= footer) — output contract
// unchanged.
//
...
// Browser shim for Node's `process` — app code + bundled remark/vfile polyfills
// read process.env.* / process.nextTick at module eval. Ships inside the IIFE so
// every design built with the bundle runs, not just the preview cards.
const PROCESS_SHIM =
  ...
  'platform:"browser",browser:true,argv:[],version:"",versions:{}};}';
...
// next/navigation no-op stub. Next's real useRouter() throws "invariant
// expected app router to be mounted" outside a Next app — which is every
// claude.ai/design render and every preview. Aliasing it to a no-op router in
// the bundle makes TabBar/AppShell (and any router-reading component) render
// everywhere instead of throwing. App-router only; navigation is inert in a DS.
const NEXT_NAV_STUB = `
const noop = () => {};
const router = { push: noop, replace: noop, back: noop, forward: noop, prefetch: noop, refresh: noop };
...
`;
const nextNavStub = {
  name: 'next-navigation-stub',
  setup(b) {
    b.onResolve({ filter: /^next\/navigation$/ }, () => ({ path: 'next-navigation', namespace: 'next-nav-stub' }));
    b.onLoad({ filter: /.*/, namespace: 'next-nav-stub' }, () => ({ contents: NEXT_NAV_STUB, loader: 'js' }));
  },
};

function sharedBuildOptions({ nodePaths, tsconfig }) {
  const pathsPlugin = tsconfig ? tsconfigPathsPlugin(tsconfig) : null;
  const plugins = [reactShim, nextNavStub];
